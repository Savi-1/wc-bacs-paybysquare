...
9. **`assets/` does not go into trunk.** `assets/blueprints/` (the Playground Live Preview blueprint, its guide notice and the demo QR mu-plugin) belongs in the SVN **`assets/`** directory, next to the banner, icons and screenshots. Both mu-plugins are inlined into `blueprint.json` as `writeFile` steps so the preview does not depend on `ps.w.org` CDN caching; the separate `.php` files are the readable copies of that code, so change both when changing one. Treat git as the source of truth: edit here, then commit to SVN `assets/`. In April 2026 the blueprint was iterated directly in SVN and git fell behind; the two were re-synced from SVN on 2026-09-04. Before copying `assets/` to SVN, `svn diff` first.
